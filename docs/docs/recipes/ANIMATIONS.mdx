...
You can use `AvoidSoftInput.onSoftInputShown`, `AvoidSoftInput.onSoftInputHidden` and `AvoidSoftInput.onSoftInputAppliedOffsetChange` methods to listen for soft input events.

If you plan to use those methods in `useEffect`, you can instead use shortcut hooks:

- `useSoftInputShown`
- `useSoftInputHidden`
- `useSoftInputAppliedOffsetChange`
...
`AvoidSoftInputView` receives events through `onSoftInputShown`, `onSoftInputHidden`, `onSoftInputAppliedOffsetChange` callbacks.

Based on those callbacks, you can apply animations in similar way as when using `useSoftInputShown`, `useSoftInputHidden` and `useSoftInputAppliedOffsetChange` hooks.
